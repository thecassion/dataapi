...
def data_processing(engine):
    ...
    eid['calculated_ageDiff'] = eid.apply(lambda eid: relativedelta(
        eid.date_blood_taken, eid.date_of_birth).months, axis=1)
    eid['calculated_ageDiff'] = eid['calculated_ageDiff'].abs()
    eid.loc[(eid.calculated_ageDiff >= 0) & (
        eid.calculated_ageDiff <= 2), "tranche_age"] = "0_2"
    eid.loc[(eid.calculated_ageDiff > 2) & (
        eid.calculated_ageDiff <= 12), "tranche_age"] = "2_12"
    eid.Office.fillna("---", inplace=True)
    eid.hospital.fillna("---", inplace=True)
    return eid
...
def testing_eid_total(engine, year=None, office=None, hospital=None):
    ...
    # put it in records
    pcr_eid = StatusEid.pcr_status(
        eid, year, office, hospital)
