...
class AGYW_Analysis:
    ...
    def data_actif_served(self):
        ...
        __actif_served.number_condoms_reception_in_the_interval.fillna(
            0, inplace=True)
        ...
        __actif_served.number_gynecological_care_date_in_the_interval.fillna(
            0, inplace=True)
        __actif_served.number_vbg_treatment_date_in_the_interval.fillna(
            0, inplace=True)
        ...
        __actif_served.number_condoms_sensibilization_date_in_the_interval.fillna(
            0, inplace=True)
        __actif_served.number_prep_awareness_date_in_the_interval.fillna(
            0, inplace=True)
        __actif_served.number_prep_reference_date_in_the_interval.fillna(
            0, inplace=True)
        ...
        __actif_served['curriculum_detailed'] = __actif_served.nbre_pres_for_inter.map(
            curriculum_detailed)
        __actif_served['curriculum'] = __actif_served.nbre_pres_for_inter.map(
            curriculum)

        __actif_served['condom'] = __actif_served.apply(
            lambda df: condom(df), axis=1)

        __actif_served['hts_awareness'] = __actif_served.number_hiv_test_awareness_date_in_the_interval.map(
            hts_awareness)
        __actif_served['hts_type_test'] = __actif_served.type_of_test_vih.map(
            type_test)
        __actif_served['hts'] = __actif_served.number_test_date_in_the_interval.map(
            hts)
        ...
        __actif_served['ps_1014'] = __actif_served.apply(
            lambda df: prim_1014(df), axis=1)
        __actif_served['ps_1519'] = __actif_served.apply(
            lambda df: prim_1519(df), axis=1)
        __actif_served['ps_2024'] = __actif_served.apply(
